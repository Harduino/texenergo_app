...
aws s3 cp app/ s3://app.texenergo.com/app/ --recursive --exclude "*" --include "*.mp3" --region eu-west-1 --acl public-read --cache-control max-age=86400
...
aws s3 cp assets/plugin/tinymce/ s3://app.texenergo.com/assets/plugin/tinymce/ --recursive --region eu-west-1 --acl public-read --cache-control max-age=86400
