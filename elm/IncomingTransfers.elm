...
type Msg = FetchIncomingTransfers
...
type NewIncomingTransferFields = Number
  | ITDate
  | Description
  | Amount
...
type alias IncomingTransfer = { id : String
  , number : String
  , total : Float
  , partner : Partner
  , date : Date
...
fetchIncomingTransfers m p f =
  ...
  in
  Http.send FetchedIncomingTransfers (Http.get endpoint incomingTransfersDecoder)
...
update msg m =
  let
    ...
    setDate str nit =
      { nit | date = str } --Date.fromString str}
  ...
  in
  case msg of
    FetchIncomingTransfers -> ( m, Cmd.none )
    ...
    FetchedIncomingTransfers (Result.Err _) -> (m, Cmd.none)
      ...
      case f of
        Amount ->
          case String.toFloat v of
            Ok goodFloat -> ( { m | newIncomingTransfer = (setAmount goodFloat m.newIncomingTransfer) }, Cmd.none )
            Err _ -> ( { m | newIncomingTransfer = (setAmount 0 m.newIncomingTransfer) } , Cmd.none )
        Number -> ( { m | newIncomingTransfer = (setNumber v m.newIncomingTransfer) }, setPicker "Called picker" )
        ITDate -> ( m, Cmd.none )
        Description -> ( { m | newIncomingTransfer = (setDescription v m.newIncomingTransfer) }, Cmd.none )
...
init fs =
  let
    ...
    initNewIncomingTransfer = NewIncomingTransfer "" "" "" 0 initPartner

    initModel : Model
    initModel = Model fs [] 1 False "" False initNewIncomingTransfer initPartnerConf
  in
